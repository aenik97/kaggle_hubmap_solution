...
def Conv2dBnReLU(
    in_channel, out_channel, kernel_size=3, padding=1, stride=1, dilation=1
):
            ...
            [
                nn.Sequential(
                    ...
                    *(
                        ...
                        if use_bn_mlp
                        else (nn.Conv2d(dim, decoder_dim, 1, padding=0, bias=True),)
                    ),
...
class daformer_conv3x3(DaformerDecoder):
    def __init__(self, **kwargs):
        super(daformer_conv3x3, self).__init__(fuse="conv3x3", **kwargs)
...
if 0:
    ...
    print(m)
    feature = [
        torch.rand(1, c, 64 // (2**i), 64 // (2**i))
        for i, c in enumerate([32, 64, 160, 256])
    ]
    y, out = m(feature)
    print(y.shape)
